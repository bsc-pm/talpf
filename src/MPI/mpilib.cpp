...
#error OpenMPI 2.1.1 has a buggy MPI_Win_fence
	...
	void Comm :: getRequest(void ** request) const
	{
		(*request) = malloc(sizeof(MPI_Request));
		
	}
	void Comm :: ibarrier(void * request) const
	{
		int rc = MPI_Ibarrier(m_impl->comm(), (MPI_Request *) request);
		if (MPI_SUCCESS != rc )
		{
			throw Exception("MPI ibarrier failed");
		}
