...
)
{
	lpf::Interface * i = realContext(ctx);
	if (!i->isAborted())
		i->atomic_fetch_and_add( src_slot, src_offset, dst_pid, dst_slot, dst_offset, value );
	return LPF_SUCCESS;
}
...
)
{
	lpf::Interface * i = realContext(ctx);
	if (!i->isAborted())
		i->atomic_cmp_and_swp( src_slot, src_offset, dst_pid, dst_slot, dst_offset, cmp, swp );
	return LPF_SUCCESS;
}
