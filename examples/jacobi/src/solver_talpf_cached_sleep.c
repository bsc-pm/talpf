		...
		lpf_memslot_t upper, lpf_memslot_t lower)
{
	...
	CHECK(talpf_sync(lpf, LPF_SYNC_CACHED));
}
...
double solve(HeatConfiguration *conf, int64_t rows, int64_t cols, int timesteps, void *extraData)
{
	...
	for (int t = 0; t < timesteps; ++t) {
		if ((rank == 0 || rank == nranks - 1) && t == 4){
			#pragma oss task label("sleep") \
				inout({reps1[1][C], C=1;ncb-2}) \
				inout({reps1[nrb-2][C], C=1;ncb-2}) \
				inout({reps1[0][C], C=1;ncb-2}) \
				inout({reps1[nrb-1][C], C=1;ncb-2})
		...
		if ((rank == 0 || rank == nranks - 1) && t == 4){
			...
			{   
				...
				usleep(((end.tv_sec*1e9 + end.tv_nsec) -  (start.tv_sec*1e9 + start.tv_nsec)) * 1e-2);
			}
		}
	}
#pragma oss taskwait
