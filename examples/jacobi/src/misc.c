...
int initialize(HeatConfiguration *conf, int64_t rows, int64_t cols, int64_t rowOffset)
{
	...
	err = posix_memalign((void **)&conf->matrixAux, pagesize, rows*cols*sizeof(double));
	if (err || conf->matrixAux == NULL) {
		fprintf(stderr, "Error: Memory cannot be allocated!\n");
		exit(1);
	}

	err = posix_memalign((void **)&conf->residual, pagesize, sizeof(uint64_t));
	if (err || conf->matrixAux == NULL) {
		fprintf(stderr, "Error: Memory cannot be allocated!\n");
		exit(1);
	}
	...
	initializeMatrix(conf, conf->matrixAux, rows, cols, rowOffset);
...
static void readParameters(int argc, char **argv, HeatConfiguration *conf)
{
	...
	while ((c = getopt_long(argc, argv, "ho:f:s:r:c:t:b:R:C:d:Wv", long_options, &index)) != -1) {
		switch (c) {
			case 'h':
