...
static void update_particles(particles_block_t *particles, forces_block_t *forces, particles_block_t *remote, const int num_blocks, const float time_interval);
...
static void update_particles_block(particles_block_t *particles, forces_block_t *forces, particles_block_t *remote, const float time_interval);
...
void nbody_solve(nbody_t *nbody, const int num_blocks, const int timesteps, const float time_interval)
{
	...
	for (int t = 0; t < timesteps; t++) {
		...
		update_particles(local, forces, remote1, num_blocks, time_interval);
	}
...
void update_particles(particles_block_t *particles, forces_block_t *forces, particles_block_t *remote ,const int num_blocks, const float time_interval)
{
	for (int i = 0; i < num_blocks; i++) {
		update_particles_block(particles+i, forces+i, remote+i , time_interval);
	}
}
...
void update_particles_block(particles_block_t *particles, forces_block_t *forces, particles_block_t *remote, const float time_interval)
{
